...
class Individual:
    ...
    def crossover(self, other):
        ...
        tmp = other.x[alpha:len(self.x)] + self.x[0:alpha]
        other.x=self.x[alpha:len(self.x)] + other.x[0:alpha]
        self.x=tmp
        self.fit=None
        other.fit=None
    def mutate(self):
        ...
        for i in range(self.latticeLength):
            if self.sigma > self.uniprng.random():
                self.x[i] = self.uniprng.randint(0,self.numParticleTypes - 1)

        self.fit=None

    def evaluateFitness(self):
        if self.fit == None: self.fit=self.__class__.fitFunc(self.x,self.interactionEnergyMatrix,self.selfEnergyVector)
